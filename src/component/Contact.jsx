...
function Contact() {
  ...
  const [activeTab, setActiveTab] = useState("dic");
  ...
  return (
    <div className="p-4 mt-20 max-w-7xl mx-auto">
      ...
      <button
        onClick={() => navigate("/")}
        className="fixed top-20 left-4 flex items-center text-[#ffffff] hover:text-[#e8e8f7] bg-indigo-500 shadow-md rounded-md px-3 py-2 z-50"
      >
      ...
      {/* Toggle Buttons */}
      <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-6 mt-6 sm:mt-0">
        <button
          onClick={() => setActiveTab("dic")}
          className={`w-fit sm:w-auto px-4 py-2 rounded-md font-semibold shadow  ${
            activeTab === "dic"
              ? "bg-[#3f1063] text-white"
              : "bg-gray-200 text-[#3f1063] hover:bg-gray-300"
          }`}
        >
          DIC
        </button>
      ...
      {activeTab === "dic" && (
        <div className="overflow-x-auto">
          <table className="min-w-full border-collapse border border-gray-400 text-sm sm:text-xs md:text-sm">
            <thead>
              <tr className="bg-[#3f1063] text-white text-left">
              ...
              {dicData.map((row, index) => (
                ...
                >
                  {row.map((cell, i) => (
                    <td
                      key={i}
                      className="border border-gray-400 px-2 py-1 break-words"
                    >
                      {cell}
                    </td>
                  ))}
      ...
      {activeTab === "social" && (
        <div className="flex flex-col items-center gap-4 mt-10">
          <p className="text-lg sm:text-xl font-semibold text-[#3f1063]">
            Follow Us On
          </p>
          <div className="flex gap-8">
            <a
              href="https://www.linkedin.com/company/industriesdepartmentbihar"
              target="_blank"
              rel="noopener noreferrer"
            >
              <Linkedin className="text-[#0e76a8] w-8 h-8 hover:scale-110 transition-transform" />
            </a>
            <a
              href="https://x.com/industriesbihar "
              target="_blank"
              rel="noopener noreferrer"
            >
              <Twitter className="text-[#1DA1F2] w-8 h-8 hover:scale-110 transition-transform" />
            </a>
            <a
              href="https://www.instagram.com/industriesdeptbihar"
              target="_blank"
              rel="noopener noreferrer"
            >
              <Instagram className="text-[#4267B2] w-8 h-8 hover:scale-110 transition-transform" />
            </a>
            <a
  href=" https://state.bihar.gov.in/industries/CitizenHome.html"
  target="_blank"
  rel="noopener noreferrer"
>
  <Globe className="text-green-600 w-8 h-8 hover:scale-110 transition-transform" />
</a>
          </div>
        </div>
      )}
